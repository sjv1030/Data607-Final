...
Below is my final project for Data 607. Some new features used that weren't covered in class were `SQLite`, `data.table`, and `PerformanceAnalytics`.
...
ff$X1 <- as.Date.character(ff$X1,format = "%Y%m%d")
ff <- as.xts(ff[,-1],order.by = ff$X1)
names(ff)[6] <- "risk.free"
...
names(wiki[[1]])
sp500 <- wiki[[1]] %>% select("Ticker symbol","Security","GICS Sector","GICS Sub Industry")

names(wiki[[2]])
add_memb <- wiki[[2]][-(0:2),] %>% select("X2","X3","X4","X5")
...
for (i in 1:n){
  ...
  rollingbeta <- rollapply(tmp_df,
                           ...
                           FUN = function(Z)
                           {
                             t = lm(formula=exrt~spx.rf-1, data = as.data.frame(Z), na.rm=T);
...
1. Limitations in my dataset
  + My dataset isn't as exhaustive and suffers from survivorship basis
